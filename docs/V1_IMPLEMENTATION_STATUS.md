# VideoGit V1/V1.5 and WebRTC peer MVP implementation status
...
An additive V2 WebRTC slice lets one desktop host a project for multiple connected editors. Native
Git bundles carry complete commits/branches/tags and footage stays outside Git as encrypted,
resumable, SHA-256-verified chunks. Received files are deduplicated in a local content-addressed store
and relinked for commit preview. Pull fetches the host's latest committed refs, fast-forwards clean
branches, preserves divergence under peer-qualified branches, and Push rejects stale or
...
| Branch from historical commit and safe local replacement | clean branch-from-revision switches HEAD/INDEX/WORKING together; the confirmed replacement action resolves the current branch's newest commit, writes it into local WORKING, resets staged state, and preserves history/media | application integration and packaged Electron Playwright | Automated |
...
| Resolve sync and historical-branch browser flows | `tests/e2e/workflow.spec.ts` launches the real packaged Electron entry with an isolated real Git repository and watched OTIO file | two Playwright tests cover automatic detection through commit preview, plus old-commit branching/switch and newest-commit replacement | Automated |
...
| Offline core operation | local editing, history, staging, merge, preview, and export use local files, native Git, and Electron IPC only; collaboration is additive | core/integration/E2E tests run without a hosted project dependency | Automated |
| Multi-editor WebRTC clone/pull/push | native Git bundles cross authenticated RTCDataChannels; two independent peers import and later pull the newest host refs while ref updates retain dirty/stale/non-fast-forward guards | `tests/collaboration.integration.test.ts` exercises a real signaling server and real WebRTC peer connections | Automated |
...
| Encrypted WebRTC pairing | WebRTC DTLS protects the channel and a random 256-bit invite secret independently authenticates/AES-256-GCM encrypts every application RPC; 16 KiB framing applies bounded backpressure | collaboration integration transfers a multi-frame asset through real Node WebRTC peers | Automated for protocol; external security review pending |
| Signaling-only deployment | standalone WebSocket service validates room messages and ICE configuration, exposes `/healthz`, and never accepts project payloads; Docker artifact supports WSS deployment behind TLS termination | signaling build and health smoke test plus collaboration integration | Automated locally; production deployment is operator-owned |
| Desktop Host/Join/Pull/Push workflow | typed IPC exposes only collaboration use cases; dashboard joins a peer and editor shows WebRTC hosting, pairing, connected-editor count, latest pull, and transfer progress | packaged Electron Playwright launches two isolated app instances, joins, then pushes a real branch | Automated |
...
- Media received from a WebRTC peer is stored under `v1-data/media/sha256/` by its full content digest.
